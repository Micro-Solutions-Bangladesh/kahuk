...
<li class="media comment">
	<a id="c{$comment_id}"></a>
	{checkActionsTpl location="tpl_kahuk_story_comments_single_start"}
...
{* Redwine: Fix the Negative votes to remove comment in Admin Panel -> Settings -> Comments -> Negative votes to remove comment. See  https://github.com/Pligg/pligg-cms/commit/68a52f2e77cec7f95d5775444d213d1a6419e121 *}
		...
		<div class="media-body comment-content" id="wholecomment{$comment_id}">
			{if $user_logged_in == $user_userlogin || $isadmin eq 1}
				<div class="btn-group pull-right admin-links">
					...
					<ul class="dropdown-menu">
						{if $isadmin eq 1}
							<li><a href="{$kahuk_base_url}/admin/admin_users.php?mode=view&user={$user_userlogin}"><i class="fa fa-user"></i> {#KAHUK_Visual_Comment_Manage_User#} {$user_userlogin}</a></li>
							<li><a href="{$edit_comment_url}"><i class="fa fa-edit"></i> {#KAHUK_Visual_Comment_Edit#}</a></li>
							<li><a href="{$delete_comment_url}"><i class="fa fa-trash-o"></i> {#KAHUK_Visual_Comment_Delete#}</a></li>
						{elseif $user_logged_in == $user_userlogin}
							<li><a href="{$edit_comment_url}"><i class="fa fa-edit"></i> {#KAHUK_Visual_Comment_Edit#}</a></li>
						{/if}
						{checkActionsTpl location="tpl_kahuk_story_comments_admin"}
					</ul>
				</div>
			{/if}
			<p class="comment-data">
				<span class="comment-author">
					{if $is_anonymous}
						{#KAHUK_Visual_Comment_Manage_Unregistered#}
					{/if}
					<a href="{$user_view_url}">{$user_username}</a> {checkActionsTpl location="tpl_kahuk_story_comments_extra"}
				...
				<span class="comment-reply">
					{if $current_userid neq 0} 
						<a href="#" onclick="show_replay_comment_form('{$comment_id}')" id="comment-reply-{$comment_id}" >{#KAHUK_Visual_Comment_Reply#}</a>
					{/if}
				</span>
				{if $comment_votes lt 0}
					<span class="label label-danger comment-hide">
						<span id="show_hide_comment_content-{$comment_id}"> <a href = "javascript://"  onclick="var replydisplay=document.getElementById('comment_content-{$comment_id}').style.display ? '' : 'none'; document.getElementById('comment_content-{$comment_id}').style.display = replydisplay;">{#KAHUK_Visual_Comment_Show_Hide#}</a></span>
					</span>
				{/if}
			</p>
	...
	{checkActionsTpl location="tpl_kahuk_story_comments_single_end"}
</li>
<!--/comment_show.tpl -->