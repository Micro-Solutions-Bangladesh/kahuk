...
var kahuk_hours = '{#KAHUK_Visual_Story_Times_Hours#}';
var items = Array();
var new_items = 0;
...
function to_html(data) {
	...
	if (diff > 1440) timeStr = '> 25 ' + kahuk_hours; // 1440 = 60 mins/hr * 24 hrs/day
	...
	if (data.type == 'problem')
		...
		html += '<td class="live2-type"><strong><a href="' + my_kahuk_base + '/live_unpublished.php">'+data.type+'</a></strong></td>';
	else if (data.type == 'Published')
		html += '<td class="live2-type"><strong><a href="' + my_kahuk_base + '/live_published.php">'+data.type+'</a></strong></td>';	
	else if (data.type == 'Comment')
		html += '<td class="live2-type"><strong><a href="' + my_kahuk_base + '/live_comments.php">'+data.type+'</a></strong></td>';	
	else
		html += '<td class="live2-type">'+data.type+'</td>';

	html += '<td class="live2-votes">'+data.votes+'</td>';
	html += '<td class="live2-story"><a href="' + my_kahuk_base + '/story.php?id='+data.link+'">'+data.title+'</a></td>';
	if (data.type == 'problem')
		html += '<td class="live2-who"><span class="live2-problem">'+data.who+'</span></td>';
	else if (data.uid > 0) 
		html += '<td class="live2-who"><a href="' + my_kahuk_base + '/user.php?login='+data.who+'">'+data.who+'</a></td>';
	else 
