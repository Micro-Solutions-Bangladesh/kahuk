<script type="text/javascript" language="javascript">
		...
		$(document).ready(function(){
			...
			{ 
				var	url = my_kahuk_url+"/load_data_for_groups.php";
		   ...
		   if(Pager_setting==2){
			...
			$(window).scroll(function(){
				...
				if(parseInt(total_row_for_group)>=count)  
				$(".group_container:last").after("<div class='btn btn-default contine_read_story'>{/literal}{#KAHUK_Continue_Reading#}{literal}</div>"); 
				...
				$(".contine_read_story").live("click", function(){
					if(parseInt(total_row_for_group)>count){
						...
						$(this).hide();
						$(".group_container:last").after("<div class='btn btn-default no_stories_left'>{/literal}{#KAHUK_No_More_Articles#}{literal}</div>"); 
					}
				});
		   }
		})
