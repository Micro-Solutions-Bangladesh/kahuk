...
<script type="text/javascript">
	...
	{
	var my_kahuk_url={/literal}"{$kahuk_base_url}"{literal};
	...
	var pageSize={/literal}"{if !empty($scrollpageSize)}{$scrollpageSize}{/if}"{literal};
		...
		{ 
			...
			if(page_name=="index" || page_name=="new" || page_name=="published"){
				...
				var searchorder={/literal}"{if !empty($searchOrder)}{$searchOrder}{/if}"{literal};
			 	...
			 	url = my_kahuk_url+"/load_data.php";
			 }
	   ...
	   if(Pager_setting==2){
		$(window).scroll(function(){
			...
			if(parseInt(total_row)>count)  
			$(".stories:last").after("<div class='btn btn-default contine_read_story'>{/literal}{#KAHUK_Continue_Reading#}{literal}</div>"); 
			...
			$(".contine_read_story").live("click", function(){
				if(parseInt(total_row) > count){
					...
					$(this).hide();
					$(".stories:last").after("<div class='btn btn-default no_stories_left'>{/literal}{#KAHUK_No_More_Articles#}{literal}</div>"); 
				}
			});
	   }
	});
