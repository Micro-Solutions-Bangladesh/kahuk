...
{*Redwine: to only link the title when the story content is not empty*}
			...
			<div class="subtext subtext-spc">
				...
				<span itemprop="name">
				<a href="{$submitter_profile_url}">{$link_submitter}{if $submitter_rank neq ''} (#{$submitter_rank}){/if}</a> {checkActionsTpl location="tpl_plikli_story_submitter"} 
				</span></span>
		...
		{/if}
				...
				{if $enable_tags}
                    ...
                    <div style="float:left;margin-top:10px;">
						<i class="fa fa-tag fa-lg faicons"></i>
						{section name=thistag loop=$tag_array}
							{if $tag_array[thistag] neq ''}
								<span class="tag"><a href="{$tags_url_array[thistag]}">{$tag_array[thistag]}</a></span>
							{/if}
						{/section}
					{/if}
                    </div><br clear="all" />
...
{* Redwine: this is only applied when a URL is not needed to submit a story. It loads the Editorial from the config table*}
				...
				<div style="float:left;margin-top:10px;"><span id="ls_comments_url-{$link_shakebox_index}">
					{if $story_comment_count eq 0}
						<i class="fa fa-comment fa-lg faicons"></i> <span id="linksummaryDiscuss"><a href="{$story_url}#discuss" class="comments">{#PLIKLI_MiscWords_Discuss#}</a>&nbsp;</span>
					{/if}
					{if $story_comment_count eq 1}
						<i class="fa fa-comment fa-lg faicons"></i> <span id="linksummaryHasComment"><a href="{$story_url}#comments" class="comments2">{$story_comment_count} {#PLIKLI_MiscWords_Comment#}</a>&nbsp;</span>
					{/if}
					{if $story_comment_count gt 1}
						<i class="fa fa-comment fa-lg faicons"></i> <span id="linksummaryHasComment"><a href="{$story_url}#comments" class="comments2">{$story_comment_count} {#PLIKLI_MiscWords_Comments#}</a>&nbsp;</span>
					{/if}
				</span> </div>
				{if $user_logged_in}  
					<div style="float:left;margin-top:10px;"><iframe height="0px;" width="0px;" frameborder="0" name="add_stories"></iframe>
						...
						<a id="{if $link_mine eq 0}add{else}remove{/if}" linkid="{$link_id}" title="{$title_short}" class="favorite" >{if $link_mine eq 0}{#PLIKLI_MiscWords_Save_Links_Save#}{else}{#PLIKLI_MiscWords_Save_Links_Remove#}{/if}</a>
					</span>&nbsp;
					<span id="stories-{$link_shakebox_index}" class="label label-success" style="display:none;line-height:1em;">{#PLIKLI_MiscWords_Save_Links_Success#}</span></div>
				{/if}
				{if $link_shakebox_currentuser_votes eq 1 && $link_shakebox_currentuser_reports eq 1}
					...
					<span id="linksummaryUnvote"><a href="javascript:{$link_shakebox_javascript_unvote}">{#PLIKLI_Visual_Unvote_For_It#}</a></span>&nbsp; </div>
				{/if}
