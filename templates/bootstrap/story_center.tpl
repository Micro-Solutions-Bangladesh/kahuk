...
<ul class="nav nav-tabs" id="storytabs">
	<li class="active"><a data-toggle="tab" href="#comments"><i class="fa fa-comments"></i> {#PLIKLI_Visual_Story_Comments#}</a></li>
	{if count($voter) neq 0}<li><a data-toggle="tab" href="#who_voted"><i class="fa fa-thumbs-up"></i> {#PLIKLI_Visual_Story_Who_Upvoted#}</a></li>{/if}
	{if count($downvoter) neq 0}<li><a data-toggle="tab" href="#who_downvoted"><i class="fa fa-thumbs-down"></i> {#PLIKLI_Visual_Story_Who_Downvoted#}</a></li>{/if}
	{if count($related_story) neq 0}<li><a data-toggle="tab" href="#related"><i class="fa fa-tag"></i> {#PLIKLI_Visual_Story_RelatedStory#}</a></li>{/if}
	{checkActionsTpl location="tpl_plikli_story_tab_end"}
</ul>
...
<div id="tabbed" class="tab-content">
	...
	<div class="tab-pane fade active in" id="comments" >
		...
		<ol class="media-list comment-list">
			{checkActionsTpl location="tpl_plikli_story_comments_individual_start"}
				{$the_comments}
			{checkActionsTpl location="tpl_plikli_story_comments_individual_end"}
			...
			{else}
				...
				<div align="center" class="login_to_comment">
					<br />
					<h3><a href="{$login_url}">{#PLIKLI_Visual_Story_LoginToComment#}</a> {#PLIKLI_Visual_Story_Register#} <a href="{$register_url}">{#PLIKLI_Visual_Story_RegisterHere#}</a>.</h3>
				</div>
				{checkActionsTpl location="anonymous_comment_form_end"}
			{/if}
		</ol>
		{checkActionsTpl location="tpl_plikli_story_comments_end"}
	</div>
	
	{if count($voter) neq 0}
		<div class="tab-pane fade" id="who_voted">
			<h3>{#PLIKLI_Visual_Story_WhoVoted#}</h3>
			{checkActionsTpl location="tpl_plikli_story_who_voted_start"}
			...
			{checkActionsTpl location="tpl_plikli_story_who_voted_end"}
		</div>
	{/if}
	{if count($downvoter) neq 0}
		<div class="tab-pane fade" id="who_downvoted">
			<h3>{#PLIKLI_Visual_Story_Who_Downvoted_Story#}</h3>
			{checkActionsTpl location="tpl_plikli_story_who_downvoted_start"}
			...
			{checkActionsTpl location="tpl_plikli_story_who_downvoted_end"}
		</div>
	{/if}
	{if count($related_story) neq 0}
		<div class="tab-pane fade" id="related">
			<h3>{#PLIKLI_Visual_Story_RelatedStory#}</h3>
			{checkActionsTpl location="tpl_plikli_story_related_start"}
			<ol>
			...
			{checkActionsTpl location="tpl_plikli_story_related_end"}
		</div>
	{/if}
