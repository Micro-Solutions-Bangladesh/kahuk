	...
	*************************************}
	...
	{if $submit_error eq 'cantedit'}
		...
		<p>You cannot edit link after {$edit_time} minutes!</p>
			<br/>
			<form id="thisform">
				<input type="button" onclick="gPageIsOkToExit=true; document.location.href='{$toredirect}';" value="{#KAHUK_Visual_Submit3Errors_Back#}" class="btn btn-primary" />
			</form>
		</div>
	{/if}
	{if $submit_error eq 'invalidurl'}
		<div class="alert alert-danger">
			...
			<br/>
			<form id="thisform">
				<input type="button" onclick="javascript:gPageIsOkToExit=true;window.history.go(-1);" value="{#KAHUK_Visual_Submit2Errors_Back#}" class="btn btn-primary">
			</form>
		</div>
	{/if}
	...
	{/if}
	{checkActionsTpl location="tpl_kahuk_submit_error_2"}
	{************************************
	...
	*************************************}
	{if $submit_error eq 'badkey'}
		<div class="alert alert-danger">
			...
			<br/>
			<form id="thisform">
				<input type="button" onclick="gPageIsOkToExit=true; document.location.href='{$kahuk_base_url}/{$pagename}.php?id={$link_id}';" value="{#KAHUK_Visual_Submit3Errors_Back#}" class="btn btn-primary" />
			</form>
		</div>
	{/if}
