...
<html class="no-js" dir="{#PLIKLI_Visual_Language_Direction#}" xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
<head>
	{checkActionsTpl location="tpl_plikli_head_start"}
	
	<!-- START META -->
		{include file=$the_template"/meta.tpl"}
	<!-- END META -->
	
	<link rel="stylesheet" type="text/css" href="{$my_plikli_base}/templates/{$the_template}/css/bootstrap.no-icons.min.css">
	<link rel="stylesheet" type="text/css" href="{$my_plikli_base}/templates/{$the_template}/css/bootstrap-theme.min.css">
	...
	<link rel="alternate" type="application/rss+xml" title="RSS 2.0" href="{$my_base_url}{$my_plikli_base}/rss.php"/>
	<link rel="icon" href="{$my_plikli_base}/favicon.ico" type="image/x-icon"/>
	{checkActionsTpl location="tpl_plikli_head_end"}
</head>
<body dir="{#PLIKLI_Visual_Language_Direction#}" {if isset($body_args)}{$body_args}{/if} {checkActionsTpl location="tpl_plikli_body_onload"}>
	{if $maintenance_mode eq "true" && $user_level eq 'admin'}
		<div class="alert alert-danger" style="margin-bottom:0;"><button class="close" data-dismiss="alert">&times;</button>{#PLIKLI_Maintenance_Admin_Warning#}</div>
	{/if}
	
	{checkActionsTpl location="tpl_plikli_body_start"}
	...
	<div class="container">
		<section id="maincontent">
			<div class="row">
				{checkActionsTpl location="tpl_plikli_banner_top"}
			...
			{checkActionsTpl location="tpl_plikli_main_col_start"}
					...
					{checkActionsTpl location="tpl_plikli_above_center"}
					
					<!-- START CENTER CONTENT -->
						{include file=$tpl_center.".tpl"}
					<!-- END CENTER CONTENT -->
					
					{checkActionsTpl location="tpl_plikli_below_center"}
					{checkActionsTpl location="tpl_plikli_content_end"}
				</div><!--/span-->
	  
				{checkActionsTpl location="tpl_plikli_columns_start"}	
				...
				{if $pagename neq "submit" && $pagename neq "user" && $pagename neq "profile" && $pagename neq "user_edit" && $pagename neq "register" && $pagename neq "login"}
					...
					<!-- END RIGHT COLUMN -->
				{/if}
			{checkActionsTpl location="tpl_plikli_banner_bottom"}
			</div><!--/.row-->
		</section><!--/#maincontent-->
		{if $Auto_scroll != '2'}
			<hr>
	...
	{if $pagename eq 'advancedsearch' || $pagename eq 'submit'}
		<link type="text/css" rel="stylesheet" media="all" href="{$my_plikli_base}/templates/{$the_template}/css/datepicker.css" />
		<script type="text/javascript" src="{$my_plikli_base}/templates/{$the_template}/js/bootstrap-datepicker.js"></script>
	{/if}
	
    {if $pagename eq "topusers"}
		<script type="text/javascript" src="{$my_plikli_base}/templates/{$the_template}/js/jquery/jquery.tablesorter.js"></script>
		{literal}
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">{#PLIKLI_Visual_Login_Title#}</h4>
					</div>
						...
						<div class="control-group">
							<form id="signin" action="{$URL_login}" method="post">
								...
								<div style="login_modal_username">
									<label for="username">{#PLIKLI_Visual_Login_Username#}/{#PLIKLI_Visual_Register_Email#}</label>
								...
								<div class="login_modal_password">
									<label for="password">{#PLIKLI_Visual_Login_Password#}</label>
								...
								<div class="login_modal_remember">
									...
									<div class="login_modal_remember_label">
										<label for="remember" style="">{#PLIKLI_Visual_Login_Remember#}</label>
									</div>
								...
								<div class="login_modal_login">
									...
									<input class="btn btn-primary col-md-12" id="signin_submit" value="{#PLIKLI_Visual_Login_LoginButton#}" tabindex="4" type="submit">
								</div>
								{checkActionsTpl location="tpl_plikli_login_link"}
								<hr class="soften" id="login_modal_spacer" />
								<div class="login_modal_forgot">
									<a class="btn btn-default col-md-12" id="forgot_password_link" href="{$URL_login}">{#PLIKLI_Visual_Login_ForgottenPassword#}?</a>
								</div>
	...
	{*if $pagename eq "profile" || $pagename eq "user_edit"*}
		{* Masonry JavaScript *}
		<!--<script type="text/javascript" src="{$my_plikli_base}/templates/{$the_template}/js/masonry.min.js"></script> -->	
	{*/if*}
</body>
</html>
