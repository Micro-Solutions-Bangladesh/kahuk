...
{if !$link_summary_output && $pagename == 'index' && count($templatelite.get) == 0}
	...
	<div class="well blank_index">
		...
		<h2>{#KAHUK_Visual_Welcome_to#} {#KAHUK_Visual_Name#}</h2>
		<p style="font-size:1.0em;">{#KAHUK_Visual_Welcome_to_Message#}</p>
		<p><a href="submit.php" class="btn btn-primary">{#KAHUK_Visual_Welcome_to_Submit_First_Story#}</a></p>
	</div>
{else}
	{$link_summary_output}
{/if}
