	...
	<fieldset>
		...
		<form method="POST" class="form-horizontal" enctype="multipart/form-data" name="image_upload_form" action="{$edit_form_action}">
			<div class="control-group">
				<label class="control-label">{#PLIKLI_Visual_Profile_UploadAvatar2#}</label>
				<div class="controls">
					...
					<button type="submit" value="{#PLIKLI_Visual_Group_Avatar_Upload#}" name="action" class="btn btn-primary"><i class="fa fa-white fa-picture-o"></i> {#PLIKLI_Visual_Group_Avatar_Upload#}</button>
				</div>
			</div>
		</form> 
			...
			<div class="control-group">
				...
				<div class="controls">
					<select name="group_privacy" class="form-control" onchange="document.getElementById('group_email').style.display=this.selectedIndex==0 ? 'none' : 'block';">
						<option {if $group_privacy eq 'public'}SELECTED{/if} value = "public">{#PLIKLI_Visual_Submit_Group_Public#}</option>
						<option {if $group_privacy eq 'private'}SELECTED{/if} value = "private">{#PLIKLI_Visual_Submit_Group_Private#}</option>
						<option {if $group_privacy eq 'restricted'}SELECTED{/if} value = "restricted">{#PLIKLI_Visual_Submit_Group_Restricted#}</option>
					</select>
					<div id='group_email' {if $group_privacy eq 'public'}style="display:none;"{/if}>
						...
						<p class="help-inline">{#PLIKLI_Visual_Submit_Group_Notify#}</p>
					</div>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					...
					<p class="help-inline">{#PLIKLI_Visual_Group_Submit_NoOfVoteInstruction#}</p>
				</div>
			</div>
			<div class="form-actions">
				<input type="submit" value="{#PLIKLI_Visual_Group_Edit#}" class="btn btn-primary" name="action" />
				<input type="button" onclick="history.go(-1)" value="{#PLIKLI_Visual_View_User_Edit_Cancel#}" class="btn btn-default" />
			</div>
		</form><!--/.form-horizontal -->
	</fieldset>
{else}
	{#PLIKLI_Visual_Group_Admin_Error#}
{/if}
<!--/edit_group_center.tpl -->