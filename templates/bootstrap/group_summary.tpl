...
<div class="group_container">
	...
	<div style="float:left;margin:0 0 10px 10px;" class="col-md-7">
		...
		<span class="group_members"> with {$group_members} {#KAHUK_Visual_Group_Member#}</span>
		<br />
		...
		{if $pagename eq 'group_story'}
					...
					{if $is_group_member eq 0}
						{if $join_group_url neq '' || $join_group_privacy_url neq ''}
							{if $group_privacy eq 'public'}
								<a class="btn btn-default" href="{$join_group_url}" ><i class="fa fa-check"></i> {#KAHUK_Visual_Group_Join#}</a>
							{else}
								<a class="btn btn-default" href="{$join_group_privacy_url}" ><i class="fa fa-check"></i> {#KAHUK_Visual_Group_Join#}</a>
							{/if}
						{/if}
					{else}
						{if $is_member_active eq 'active'}
							<a class="btn btn-default" href="{$unjoin_group_url}" ><i class="fa fa-times"></i> {#KAHUK_Visual_Group_Unjoin#}</a>
						{/if}
						{if $is_member_active eq 'inactive'}
							<a class="btn btn-default" href="{$join_group_withdraw}" ><i class="fa fa-times"></i> {#KAHUK_Visual_Group_Withdraw_Request#}</a>
						{/if}	
					{/if}
				{/if}
			{/if}
			...
			{if !empty($is_group_admin) && $is_group_admin eq '1' || $isAdmin eq '1' || $isModerator eq '1' || $is_gr_Admin eq '1'}
				<a class="btn btn-default" href="{$group_edit_url}"><i class="fa fa-edit"></i> {#KAHUK_Visual_Group_Text_edit#}</a>
				{if $allow_groups_avatar eq 'true'}
				<a class="btn btn-default" href="#groupavatar" data-toggle="modal"><i class="fa fa-picture-o"></i> {#KAHUK_Visual_Group_Avatar_Upload#}</a>
				{/if}
				...
				<a class="btn btn-danger" onclick="return confirm('{#KAHUK_Visual_Group_Delete_Confirm#}')" href={$group_delete_url}><i class="fa fa-white fa-trash-o"></i> {#KAHUK_Visual_Group_Text_Delete#}</a>
						...
						<form method="POST" enctype="multipart/form-data" name="image_upload_form" action="{$form_action}">
							<div class="modal-content">
								...
								<div class="modal-footer">
									...
									<button type="submit" name="action" class="btn btn-primary"><span class="fa fa-white fa fa-picture"></span> {#KAHUK_Visual_Group_Avatar_Upload#}</button>
