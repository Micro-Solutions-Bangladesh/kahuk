	...
	<fieldset>
		...
		<form action="{$URL_submit_groups}" method="post" name="thisform" id="thisform" class="form-horizontal" enctype="multipart/form-data">
			...
			<div class="control-group">
				<label class="control-label">{#KAHUK_Visual_Submit_Group_Title#}:</label>
				<div class="controls">
					...
					<br /><p class="help-inline">{#KAHUK_Visual_Group_Submit_TitleInstruction#}</p>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					...
					<br /><p class="help-inline">{#KAHUK_Visual_Group_Submit_DescriptionInstruction#}</p>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					<select name="group_privacy" class="form-control" onchange="document.getElementById('group_email').style.display=this.selectedIndex==0 ? 'none' : 'block';">
						<option value = "public" {if !empty($templatelite.post.group_privacy) && $templatelite.post.group_privacy=='public'}selected{/if}>{#KAHUK_Visual_Submit_Group_Public#}</option>
						<option value = "private" {if !empty($templatelite.post.group_privacy) && $templatelite.post.group_privacy=='private'}selected{/if}>{#KAHUK_Visual_Submit_Group_Private#}</option>
						<option value = "restricted" {if !empty($templatelite.post.group_privacy) && $templatelite.post.group_privacy=='restricted'}selected{/if}>{#KAHUK_Visual_Submit_Group_Restricted#}</option>
					</select>
					<div id='group_email' {if isset($templatelite.post.group_privacy) && $templatelite.post.group_privacy == 'public' || empty($templatelite.post.group_privacy)}style="display:none;"{/if}>
						...
						{#KAHUK_Visual_Submit_Group_Notify#}
					</div>
					<br />{#KAHUK_Visual_Group_Submit_PrivacyInstruction#}
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">{#KAHUK_Visual_Submit_Group_Mail_Friends#}:</label>
				<div class="controls">
					...
					<br /><p class="help-inline">{#KAHUK_Visual_Group_Submit_Mail_Friends_Desc#}</p>
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">{#KAHUK_Visual_Submit_Group_vote_to_publish#}:</label>
				<div class="controls">
					...
					<br /><p class="help-inline">{#KAHUK_Visual_Group_Submit_NoOfVoteInstruction#}</p>
				</div>
			</div>
			<div class="form-actions">
				<input type="submit" value="{#KAHUK_Visual_Submit_Group_create#}" class="btn btn-primary" />
				<input type="button" onclick="history.go(-1)" value="{#KAHUK_Visual_View_User_Edit_Cancel#}" class="btn btn-default" />
			</div>
		</form>
	</fieldset>
{else}
