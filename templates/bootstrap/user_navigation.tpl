...
{checkActionsTpl location="tpl_kahuk_profile_begin"}
	<div class="col-md-12 user_navigation">
		<h1 class="user_name">
			{if $UseAvatars neq "0" && $pagename == "user_edit"}
				<a href="#profileavatar" data-toggle="modal">
					<div class="img-thumbnail avatar_thumb">
						...
						{/php}
						{checkActionsTpl location="tpl_kahuk_profile_avatar_start"}
						<img class="profile_avatar" src="{$Avatar.large}" alt="Avatar" />
						{checkActionsTpl location="tpl_kahuk_profile_avatar_end"}
				...
				<div class="modal fade" id="profileavatar">
					<div class="modal-dialog">
						...
						{checkActionsTpl location="tpl_kahuk_profile_avatar_upload_start"}
							<form method="POST" enctype="multipart/form-data" name="image_upload_form">
								...
								<div class="modal-header">
									...
									<h4 class="modal-title">{#KAHUK_Visual_Profile_UploadAvatar2#}</h4>
								</div>
								<div class="modal-body">
									<div class="fileupload fileupload-new" data-provides="fileupload">
										<div class="fileupload-new img-thumbnail">
											<img src="{$Avatar.large}" title="{#KAHUK_Visual_Profile_CurrentAvatar#}" />
										</div>
								...
								<div class="modal-footer">
									...
									<a class="btn btn-default" data-dismiss="modal">{#KAHUK_Visual_View_User_Edit_Cancel#}</a>
									<input type="submit" name="action" class="btn btn-primary" value="{#KAHUK_Visual_Profile_AvatarUpload#}"/>
								</div>
							</form>
							{checkActionsTpl location="tpl_kahuk_profile_avatar_upload_end"}
						</div><!-- /.modal-content -->
					</div><!-- /.modal-dialog -->
				</div><!-- /.modal -->
			{elseif $UseAvatars neq "0" && $pagename == "user"}
				{if $user_login eq $user_logged_in}
					<a href="{if $UrlMethod == "2"}{$kahuk_base_url}/user/{$user_login}/edit/?avatar=edit{else}{$kahuk_base_url}/profile.php?avatar=edit{/if}">
				{/if}
				<div class="img-thumbnail avatar_thumb">
					...
					{if $user_login eq $user_logged_in}
						<a class="btn btn-default btn-xs edit-avatar" href="{if $UrlMethod == "2"}{$kahuk_base_url}/user/{$user_login}/edit/?avatar=edit{else}{$kahuk_base_url}/profile.php?avatar=edit{/if}">Edit Avatar</a>
					{/if}
				</div>
			...
			{/if}
			{checkActionsTpl location="tpl_kahuk_profile_username_start"}{$user_username|capitalize}{checkActionsTpl location="tpl_kahuk_profile_username_end"}
		</h1>
		...
		<div class="user_social">
			...
			{if $user_skype}
				<a href="callto://{$user_skype}" title="Skype {$user_username|capitalize}" rel="nofollow noopener noreferrer" target="_blank"><span class="fa-stack fa-lg opacity_reset"><i class="fa fa-circle fa-stack-2x opacity_reset" style="color:#00aaf1;"></i><i class="fa fa-skype fa-stack-1x fa-inverse opacity_reset"></i></span></a>
			{/if}
			{if $user_facebook}
				<a href="http://www.facebook.com/{$user_facebook}" title="{$user_username|capitalize} on Facebook" rel="nofollow noopener noreferrer" target="_blank"><span class="fa-stack fa-lg opacity_reset"><i class="fa fa-circle fa-stack-2x opacity_reset" style="color:#3c5b9b;"></i><i class="fa fa-facebook fa-stack-1x fa-inverse opacity_reset"></i></span></a>
			{/if}
			{if $user_twitter}
				<a href="http://twitter.com/{$user_twitter}" title="{$user_username|capitalize} on Twitter" rel="nofollow noopener noreferrer" target="_blank"><span class="fa-stack fa-lg opacity_reset"><i class="fa fa-circle fa-stack-2x opacity_reset" style="color:#2daae1;"></i><i class="fa fa-twitter fa-stack-1x fa-inverse opacity_reset"></i></span></a>
			{/if}
			{if $user_linkedin}
				<a href="http://www.linkedin.com/in/{$user_linkedin}" title="{$user_username|capitalize} on LinkedIn" rel="nofollow noopener noreferrer" target="_blank"><span class="fa-stack fa-lg opacity_reset"><i class="fa fa-circle fa-stack-2x opacity_reset" style="color:#0173b2;"></i><i class="fa fa-linkedin fa-stack-1x fa-inverse opacity_reset"></i></span></a>
			{/if}
			{if $user_googleplus}
				<a href="https://plus.google.com/{$user_googleplus}" title="{$user_username|capitalize} on Google+" rel="nofollow noopener noreferrer" target="_blank"><span class="fa-stack fa-lg opacity_reset"><i class="fa fa-circle fa-stack-2x opacity_reset" style="color:#f63e28;"></i><i class="fa fa-google-plus fa-stack-1x fa-inverse opacity_reset"></i></span></a>
			{/if}
			{if $user_pinterest}
				<a href="http://pinterest.com/{$user_pinterest}/" title="{$user_username|capitalize} on Pinterest" rel="nofollow noopener noreferrer" target="_blank"><span class="fa-stack fa-lg opacity_reset"><i class="fa fa-circle fa-stack-2x opacity_reset" style="color:#cb2027;"></i><i class="fa fa-pinterest fa-stack-1x fa-inverse opacity_reset"></i></span></a>
			{/if}
		...
		{if $Allow_Friends neq "0"}
			<div class="btn-group user_followers">
				<a class="btn btn-default btn-sm" href="{$user_url_friends}"><i class="fa fa-user"></i> {$user_following} {#KAHUK_Visual_User_Profile_View_Friends#}</a>
				<a class="btn btn-default btn-sm" href="{$user_url_friends2}"><i class="fa fa-user"></i> {$user_followers} {#KAHUK_Visual_User_Profile_Your_Friends#}</a>
				...
				{if check_for_enabled_module('simple_messaging',2.0) && $is_friend && $user_login neq $user_logged_in}
					<a class="btn btn-default btn-sm" href="{$kahuk_base_url}/module.php?module=simple_messaging&view=compose&to={$username}&return={$my_kahuk_base}%2Fuser.php%3Flogin%3D{$user_logged_in}%26view%3Dfollowers"><i class="fa fa-envelope"></i> Send Message</a>
				{/if}
				{/if}
...
{* Redwine: To fix a bug that was displaying the wrong button (follow/unfollow) under the users avatar and beside the following and followers buttons in the user profile page. See the picture http://mymonalisasmile.com/upload/follow-unfollow-buttons.jpg*}
				{elseif ($is_friend eq "" || $is_friend eq "follower") && $user_login neq $user_logged_in && $user_authenticated neq false}
					<a class="btn btn-sm btn-success" href="{$user_url_add}">{#KAHUK_Visual_User_Profile_Add_Friend#} {$user_login|capitalize}</a>
				{/if}
				...
				<div class="modal fade" id="user_search_modal">
					<div class="modal-dialog">
						<div class="modal-content user_search">
							<form action="{$my_kahuk_base}/user.php" class="form-inline" role="form" method="get" {php} global $URLMethod, $my_base_url, $my_kahuk_base; if ($URLMethod==2) print "onsubmit='document.location.href=\"{$kahuk_base_url}/user/search/\"+encodeURIComponent(this.keyword.value); return false;'";{/php}>
								...
								<div class="modal-body">
									<input type="text" name="keyword" class="form-control" autofocus="autofocus" placeholder="{#KAHUK_Visual_User_Search_Users#}">
...
<ul class="nav nav-tabs" id="profiletabs">
	...
	<li {if $user_view eq 'profile' || $user_view eq 'removefriend' || $user_view eq 'addfriend'}class="active"{/if}><a {if $user_view eq 'profile'}data-toggle="tab" href="#personal_info"{else}href="{$user_url_personal_data2}"{/if}>{#KAHUK_Visual_User_PersonalData#}</a></li>
	{if $user_login eq $user_logged_in || isset($isadmin) && $isadmin eq 1}
		<li {if $pagename eq 'user_edit'}class="active"{/if}><a href="{$URL_Profile2}">{#KAHUK_Visual_User_Setting#}</a></li>
	{/if}
	{checkActionsTpl location="tpl_kahuk_profile_sort_middle"}
	<li {if $user_view eq 'history'}class="active"{/if}><a href="{$user_url_news_sent2}">{#KAHUK_Visual_User_NewsSent#}</a></li>
	<li {if $user_view eq 'published'}class="active"{/if}><a href="{$user_url_news_published2}">{#KAHUK_Visual_User_NewsPublished#}</a></li>
	<li {if $user_view eq 'new'}class="active"{/if}><a href="{$user_url_news_unpublished2}">{#KAHUK_Visual_User_NewsUnPublished#}</a></li>
	{if $user_login eq $user_logged_in}
	<li {if $user_view eq 'draft'}class="active"{/if}><a href="{$user_url_draft2}">{#KAHUK_Visual_User_NewsDraft#}</a></li>
	<li {if $user_view eq 'scheduled'}class="active"{/if}><a href="{$user_url_scheduled2}">{#KAHUK_Visual_User_NewsScheduled#}</a></li>
	{/if}
	<li {if $user_view eq 'commented'}class="active"{/if}><a href="{$user_url_commented2}">{#KAHUK_Visual_User_NewsCommented#}</a></li>
	<li {if $user_view eq 'upvoted'}class="active"{/if}><a href="{$user_url_news_upvoted2}">{#KAHUK_Visual_UpVoted#}</a></li>
	<li {if $user_view eq 'downvoted'}class="active"{/if}><a href="{$user_url_news_downvoted2}">{#KAHUK_Visual_DownVoted#}</a></li>
{* Redwine: to only show the saved links tab to the user owner of the profile. *}
	{if $user_login eq $user_logged_in}
		<li {if $user_view eq 'saved'}class="active"{/if}><a href="{$user_url_saved2}">{#KAHUK_Visual_User_NewsSaved#}</a></li>
	{/if}
