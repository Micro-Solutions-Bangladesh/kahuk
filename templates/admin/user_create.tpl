...
var my_base_url="{$kahuk_base_url}";
var error=false;
var UserNameAjaxUrl= "{$my_kahuk_base}/kahuk-ajax.php?prefix=checkfield";
{literal}
...
// The whole function needs to be revised. As it was, though it shows that some fields are incorrect, it still submits the formand the admin lands on a blank page that is confusing! So I added the OR validation part in the username and the password
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">{#KAHUK_Visual_AdminPanel_New_User#}</h4>
			</div>
				...
				<form action="admin_users.php" method="post" id="createUserForm" class="form" onsubmit="return checkValidation()">
					<div class="modal-body">	
						<!-- Form, no buttons (buttons generated by jQuery UI dialog() function) -->
							...
							<label for="username">{#KAHUK_Visual_Register_Username#}:  ({#KAHUK_Visual_Register_Error_UserTooShort#})</label>
							...
							{/if}
							<div style="clear:both;"></div>
							<label for="password">{#KAHUK_Visual_Register_Password#}:  ({#KAHUK_Visual_Register_Error_FiveCharPass#})</label>
					...
					<div class="modal-footer">
						...
						<input type="submit" id="sbmt" class="btn btn-primary" value="Create User" />
					</div>
				</form>
			</div>
		</div><!-- /.modal-content -->
