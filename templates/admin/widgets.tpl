...
{php}
	...
	if($action == 'main' || $action == 'disable' || $action == 'enable'){
		echo '<legend>'.$main_smarty->get_config_vars('KAHUK_Visual_AdminPanel_Widget_Management').' </legend>';
		echo '<p>'.$main_smarty->get_config_vars('KAHUK_Visual_AdminPanel_Widget_Description').'</p>';
		...
		if ($_GET["status"] != 'uninstalled'){
			...
			/*
			echo '<div class="apply_widgets"><input type="submit" class="btn btn-primary" name="submit" value="'.$main_smarty->get_config_vars('KAHUK_Visual_AdminPanel_Apply_Changes').'" id="apply_changes" /></div>';
			echo '<br />';
			*/
			...
			if($widgets){
				foreach($widgets as $widget) {
					if (file_exists(KAHUKPATH . 'widgets/' . $widget->folder)){
						echo '<tr>';
						/*
						...
						*/
						...
						echo '<td style="text-align:center;vertical-align:middle;">';
						echo '<a class="btn btn-danger btn-xs" href="?action=remove&widget=' . $widget->name . '">'.$main_smarty->get_config_vars('KAHUK_Visual_AdminPanel_Module_Remove').'</a>';
						echo '</td></tr>';
					}
				}
			} else {
			...
			echo '</tbody></table>';
			/*
			echo '<div class="apply_widgets"><input type="submit" class="btn btn-primary" name="submit" value="'.$main_smarty->get_config_vars('KAHUK_Visual_AdminPanel_Apply_Changes').'" /></div>';
			*/
			echo '</form>';
		} else {
			echo '<table class="table table-bordered">';
	...
	if($action == 'readme'){
		...
		echo '<legend>'.$main_smarty->get_config_vars('KAHUK_Visual_AdminPanel_Widget_Readme').'</legend>';
		echo '<a href="'.my_kahuk_base.'/admin/admin_widgets.php"><i class="fa fa-caret-left"></i> '.$main_smarty->get_config_vars('KAHUK_Visual_AdminPanel_Widget_Return').'</a><br /><hr />';
		if(file_exists('../widgets/' . $widget . '/' . $widget . '_readme.htm')){
			include_once('../widgets/' . $widget . '/' . $widget . '_readme.htm');	
		} else {
			echo $main_smarty->get_config_vars('KAHUK_Visual_AdminPanel_Widget_Readme_Not_Found');
		}
	}	
