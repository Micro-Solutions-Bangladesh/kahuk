...
<legend>
	{if $templatelite.get.user}
		{$templatelite.get.user|sanitize:2}'s {#KAHUK_Visual_Story_Comments#}
	{else}
		{#KAHUK_Visual_AdminPanel_Comments_Legend#}
	{/if}
</legend>
...
{/if}
{if $spam_comments_count neq 0}
	<div class="alert alert-warning">
		There {if $spam_comments_count eq 1}is{else}are{/if} <strong>{$spam_comments_count} Spam {if $spam_comments_count eq 1}comment{else}comments{/if}</strong> awaiting review.<br />
		<a href="admin_comments.php?filter=spam">Click here to review {if $spam_comments_count eq 1}it{else}them{/if}.</a>
	</div>
{/if}
...
<div class="row management-tools">
	<div class="col-md-3">
		<div class="btn-group pull">
			...
			<ul class="dropdown-menu">
				<li><a onclick="set_admin_action('published')" href="#"><i class="fa fa-check-square-o"></i>{#KAHUK_Visual_AdminPanel_Publish#}</a></li>
				<li><a onclick="set_admin_action('moderated')" href="#"><i class="fa fa-exclamation-triangle"></i>{#KAHUK_Visual_AdminPanel_Moderated#}</a></li>
	...
	<form action="{$kahuk_base_url}/admin/admin_comments.php" method="get" class="form-search search-bar">
		<div class="col-md-3">
		...
		<div class="col-md-3">
			<select name="filter" class="form-control" id="comment_filter" onchange="this.form.submit()">
				<option value="all" {if $templatelite.get.filter == "all"} selected="selected" {/if}>{#KAHUK_Visual_AdminPanel_All_Comments#}</option>
...
<form name="bulk_moderate" id="user_list_form" action="{$kahuk_base_url}/admin/admin_comments.php?action=bulkmod&page={$templatelite.get.page|sanitize:2}" method="post">
	...
	<table class="table table-bordered table-condensed" width="100%" >
		<thead>
			<tr>
				...
				{checkActionsTpl location="tpl_kahuk_admin_comments_th_start"}
				...
				{checkActionsTpl location="tpl_kahuk_admin_comments_th_end"}
			</tr>
		</thead>
		{if isset($template_comments)}
			...
			<tr {if $template_comments[id].comment_status=='moderated'}class="tr_moderated"{/if}>
				...
				{checkActionsTpl location="tpl_kahuk_admin_comments_td_start"}
				<td><a href="{$kahuk_base_url}/admin/admin_users.php?mode=view&user={$template_comments[id].comment_author}" title="{$template_comments[id].comment_author}'s Profile" id="comment-{$template_comments[id].comment_id}-author">{$template_comments[id].comment_author}</a> {checkActionsTpl location="tpl_kahuk_admin_comments_extra"}</td>
				<td style="text-align:justify;">
					<a href="{$kahuk_base_url}/edit.php?id={$template_comments[id].comment_link_id}&commentid={$template_comments[id].comment_id}"><i class="fa fa-edit" title="{#KAHUK_Visual_AdminPanel_Page_Edit#}" alt="{#KAHUK_Visual_AdminPanel_Page_Edit#}"></i></a>
					<a href="{$kahuk_base_url}/story.php?id={$template_comments[id].comment_link_id}#c{$template_comments[id].comment_id}" title="{$template_comments[id].comment_content_long|truncate:50:"...":true}">{$template_comments[id].comment_content}</a>
				...
				{checkActionsTpl location="tpl_kahuk_admin_comments_td_end"}
			</tr>
			{/section}
		{/if}		
	</table>
...
<div style="float:right;margin:8px 2px 0 0;">
	<a data-toggle="modal" class="btn btn-danger" href="{$kahuk_base_url}/admin/admin_delete_comments.php"><i class="fa fa-trash-o fa-white"></i> {#KAHUK_Visual_AdminPanel_Delete_Comments#}</a> 
</div>
