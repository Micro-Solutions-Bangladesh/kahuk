...
include_once(KAHUK_LIBS_DIR . 'utils.php');
...
// Load settings
include_once(KAHUK_MODULES_DIR . 'subscribe2comments/subscribe2comments_settings.php');
...
if ($_GET['unsub'] && $_GET['unsub'] == 1) {
	...
	if ($result) {
		...
		if ($subscribed) {
			$decode = md5($result->user_email . strtotime($result->user_date) .  $username . kahuk_hash());
			if ($uns_code == $decode) {
				$linkres = new Link;
				$linkres->id = $link_id;
				if ($linkres->read()) {
					...
					$main_smarty->assign('message_uns', $main_smarty->get_config_vars('KAHUK_Subscribe_2_Comments_Unsubscribed_Message'));
				} else {
					$main_smarty->assign('message_error', $main_smarty->get_config_vars('KAHUK_Subscribe_2_Comments_Invalid_Code'));
				}
			} else {
				$main_smarty->assign('message_error', $main_smarty->get_config_vars('KAHUK_Subscribe_2_Comments_Invalid_Code'));
			}
		} else {
			$main_smarty->assign('message_error', $main_smarty->get_config_vars('KAHUK_Subscribe_2_Comments_Unsubscribe_Wrong'));
		}
	} else {
		$main_smarty->assign('message_error', $main_smarty->get_config_vars('KAHUK_Subscribe_2_Comments_No_User'));
	}

	$main_smarty->config_load(subscribe2comments_kahuk_lang_conf);
