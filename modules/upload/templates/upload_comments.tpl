...
<label for="fileInput" class="control-label">{#PLIKLI_Upload_Attach#}</label>
<p class="help-block">{$upload_extensions} {#PLIKLI_Upload_Extensions_Allowed#}</p>

<script>
var uploading = '<fieldset style="border:1px solid #eee;padding:10px;margin-bottom:10px;font-weight:bold;"><h2>{#PLIKLI_Upload_Uploading#}...</h2></fieldset>';
var failed = '<fieldset style="border:1px solid #eee;padding:10px;margin-bottom:10px;font-weight:bold;"><h2>{#PLIKLI_Upload_Failed#}...</h2></fieldset>';
...
{section name=files start=0 loop=$upload_maxnumber step=1}
	...
	if (!$this->_vars['get']['commentid'])
    	...
    	$images = $db->get_results("SELECT * from " . table_prefix . "files where file_link_id='{$this->_vars['link_id']}' AND file_number='{$this->_vars['number']}' AND file_comment_id='{$this->_vars['get']['commentid']}' ORDER BY file_orig_id",ARRAY_A);
	...
	$fields = $db->get_col("SELECT file_fields from " . table_prefix . "files where file_link_id='{$this->_vars['link_id']}' AND file_number='{$this->_vars['number']}' AND file_size='orig' AND file_comment_id='{$this->_vars['get']['commentid']}'",ARRAY_A);
...
<button class="btn btn-default" onclick='if (upload_fields < {$upload_maxnumber}) add_upload_field({$upload_maxnumber}); if (++upload_fields >= {$upload_maxnumber}) this.disabled=true; return false;' {if $upload_fields>=$upload_maxnumber}disabled{/if}>{#PLIKLI_Upload_Add_File#}</button>
<br />
