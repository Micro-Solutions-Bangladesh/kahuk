...
function rss_import_showpage(){
	...
	$navwhere['text1'] = $smarty->get_config_vars('plikli_Visual_Header_AdminPanel');
	$navwhere['link1'] = getmyurl('admin', '');
	$navwhere['text2'] = $smarty->get_config_vars('plikli_Visual_Header_AdminPanel_RSSImport');
	$smarty->assign('navbar_where', $navwhere);
	$smarty->assign('posttitle', ' / ' . $smarty->get_config_vars('plikli_Visual_Header_AdminPanel_RSSImport'));
	...
	if($canIhaveAccess == 1){	
		...
		$smarty->register_function('feedsListplikliLinkFields', 'smarty_function_feedsListplikliLinkFields');
		...
		$QEIPA = array('table_name' => table_prefix . 'feed_link',  // the name of the table to use
						  'field_name' => 'plikli_field',  // the name of the field in the table
						  'key' => 'feed_link_id',  // a unique identifier for the row
						  'eip_type' => 'select');  // the type of EIP field to show 
		$smarty->assign('qeip_FeedLink_plikliField', $QEIPA);
		...
		if(!isset($_REQUEST['action'])){
			...
			if($_REQUEST['action'] == "importprebuiltfeed_go"){
				...
				if(import_prebuilt($feed)){
					redirect(my_plikli_base . '/module.php?module=rss_import');
				} else {
	...
	$x = unserialize($serialized_feed);
		...
		$sql = "Insert Into `" . table_prefix . "feeds` (`feed_id`, `feed_name`, `feed_url`, `feed_freq_hours`, `feed_votes`";
			...
			foreach($myfeed['links'] as $field_link){
				$sql = "insert into `" . table_prefix . "feed_link` (`feed_id`, `feed_field`, `plikli_field`)"; 
				$sql .= " VALUES (" . $feed_main->feed_id . ", '" . $field_link->feed_field;
				$sql .= "', '" . $field_link->plikli_field . "');";
				$db->query($sql);
			}
...
function rss_import_do_import($insideTpl = true){
	...
	$navwhere['link1'] = getmyurl('admin', '');
	$navwhere['text2'] = $main_smarty->get_config_vars('plikli_Visual_Header_AdminPanel_RSSImport');
	$navwhere['link2'] = my_plikli_base . '/module.php?module=rss_import';
	$navwhere['text3'] = $main_smarty->get_config_vars('plikli_Visual_Header_AdminPanel_RSSImport_Feeds');
	$main_smarty->assign('navbar_where', $navwhere);
	$main_smarty->assign('posttitle', ' / ' . $main_smarty->get_config_vars('plikli_Visual_Header_AdminPanel_RSSImport'));
...
function get_plikli_fields(){
	global $db;
	$sql = "select * from " . table_prefix . "feed_import_fields;";
	$plikli_fields = $db->get_results($sql);
	return $plikli_fields;
}
	
function smarty_function_feedsListplikliLinkFields($params, &$smarty){
	// get a list of plikli fields and put them into the smarty variable "eip_select" for the EIP selectbox to use
	$plikli_Fields = get_plikli_fields();
	$TheTextToReturn = "	options: {";
	$Count = 0;
	if($plikli_Fields){
		foreach ($plikli_Fields as $Field) {
...
function redirect($url, $msg = ''){
	...
	if(strpos($_SERVER['SERVER_SOFTWARE'], "IIS") && strpos(php_sapi_name(), "cgi") >= 0){
		...
		echo $main_smarty->get_config_vars($msg) . '<a href = "'.$url.'">' . $main_smarty->get_config_vars('plikli_Visual_IIS_Continue') . '</a>';
	} else {
		header('Location: ' . $url);
	}
}
