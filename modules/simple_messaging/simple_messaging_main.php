<?php  
include_once(KAHUK_LIBS_DIR.'../modules/simple_messaging/kmessaging/class.KMessaging.php');
...
function simple_messaging_showpage(){
	...
	//							$db->query('DELETE FROM `' . table_messages . '` WHERE `idMsg` = "'.$key.'"');
						...
						header("Location: ".my_kahuk_base."/module.php?module=simple_messaging&view=inbox");
						die();
					}
				}
			}
			...
			if (isset($_GET['action'])) {
				if ($db->escape(sanitize($_GET['action'],3)) == "bulkmod") {
					if(isset($_POST['submit'])) {
						...
						header("Location: ".my_kahuk_base."/module.php?module=simple_messaging&view=sent");
						die();
					}
				}
			}
	...
	if($view == 'send'){
		...
		if ($msg_result != 0){
			...
			//print 'KAHUK_MESSAGING_Error_'.$msg_result;
			//print $main_smarty->get_config_vars('KAHUK_MESSAGING_Error_'.$msg_result);

			$main_smarty->assign('message', $main_smarty->get_config_vars('KAHUK_MESSAGING_Error_'.$msg_result));
			$main_smarty->config_load(simple_messaging_kahuk_lang_conf);
			$main_smarty->assign('tpl_center', simple_messaging_tpl_path . 'error');
			$main_smarty->display($the_template . '/kahuk.tpl');
			die;
		} else {
