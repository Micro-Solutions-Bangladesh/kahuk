<div class="simple_messaging_wrapper">
	...
	<form name="bulk_moderate" action="{$my_plikli_base}/module.php?module=simple_messaging&view=inbox&action=bulkmod" method="post">
		<table class="table">
			<thead>
				<tr>
					<th>{#PLIKLI_MESSAGING_From#}</th>
					<th>{#PLIKLI_MESSAGING_Subject#}</th>
					<th style="min-width:150px">{#PLIKLI_MESSAGING_Sent#}</th>
					<th style="width:40px;">{#PLIKLI_MESSAGING_Delete#}</th>
				</tr>
			</thead>
			<tbody>
				{if $msg_array neq ""}
					{section name=themessage loop=$msg_array}
						<tr id="msg_row_{$msg_array[themessage].id}">
							<td><a href="{$my_plikli_base}/user.php?login={$msg_array[themessage].sender_name}">{$msg_array[themessage].sender_name}</a></td>
							<td>
				...
				{else}
					<tr>
						<td colspan="4"><div align="center">{#PLIKLI_MESSAGING_No_Messages#}</div></td>
					</tr>
				{/if}
			</tbody>
		</table>
		...
		{if $msg_array neq ""}
			<p align="right"><input type="submit" name="submit" value="{#PLIKLI_MESSAGING_Delete_Selected#}" class="btn btn-danger" /></p>
		{/if}
...
{checkActionsTpl location="tpl_plikli_module_simple_messaging_end"}