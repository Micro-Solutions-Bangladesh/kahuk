<div class="simple_messaging_wrapper">
	...
	<h4>{#KAHUK_MESSAGING_SendAMessageTo#}<a href="{$my_kahuk_base}/user.php?login={$msgToName}">{$msgToName}</a></h4>
	...
	<form method="get" action="module.php" role="form">
		...
		<div class="form-group">
			<label><strong>{#KAHUK_MESSAGING_Subject#}:</strong></label>
			<input class="form-control" id="msg_subject" name="msg_subject" type="text" value="{$msg_subject}" tabindex="1" required="yes">
		</div>
		<div class="form-group">
			<label><strong>{#KAHUK_MESSAGING_Message#}:</strong></label>
			<textarea class="form-control" id="msg_body" name="msg_body" tabindex="2" rows="10" required="yes" /></textarea>
		</div>
		<div class="form-group">
			<a class="btn btn-default" href="{$URL_simple_messaging_inbox}" tabindex="3">{#KAHUK_MESSAGING_Close#}</a>
			<input class="btn btn-primary" type="submit" value="{#KAHUK_MESSAGING_Send#}" tabindex="4">
		</div>
