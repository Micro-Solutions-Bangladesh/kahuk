...
{if $error || !$wordpress_key}
	<div class="alert alert-warning fade in">
		...
		{#PLIKLI_Akismet_Wrong_Key#}
	</div>
{elseif $templatelite.get.submit}
	<div class="alert alert-success fade in">
		<a data-dismiss="alert" class="close">x</a>
		{#PLIKLI_Akismet_Saved#}
	</div>
{/if}

<fieldset>
	<legend>{#PLIKLI_Akismet_manage_key#}</legend>
	<p>{#PLIKLI_Akismet_api_description#}<p>
	...
	<form method="get" action="module.php">
		...
		<div class="akismet_api_input">
			<strong>{#PLIKLI_Akismet_api_key#}</strong>
			<input type="text" name="key" class="form-control" value="{$wordpress_key}">
		</div>
		<br />
		<div class="akismet_api_submit">
			<input type="submit" name="submit" class="btn btn-primary" value="{#PLIKLI_Akismet_api_update#}">
		</div>
	</form>
	
	<br style="clear:both;" /><br />
	<a href="{$URL_akismet}">{#PLIKLI_Akismet_return#}</a>
