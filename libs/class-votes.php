...
class Vote {
	...
	function sum(){
		...
		$sql   = "SELECT sum(vote_value) FROM " . table_votes . " WHERE $where";
		$sum   = $db->get_var( $sql );
		$sum   = $this->adjust($sum);
	...
	function user_list_all_votes($cacheit = TRUE) {
		...
		if ( $get_data == TRUE ) {
			$sql = "SELECT * FROM " . table_votes . " WHERE $where";
			$output = $db->get_results( $sql );

			if ($cacheit == TRUE){
				if ($output){
					foreach ($output as $vote_row){
	...
	function listall() {
		...
		$sql = "SELECT * FROM " . table_votes . " WHERE vote_type='{$this->type}' AND vote_link_id={$this->link}";
		$output = $db->get_results( $sql );
	...
	function insert() {
		...
		// echo "class-votes -> insert -> sql:" . $sql . "<br>";
