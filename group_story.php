...
if ($requestID > 0) {
	...
	/* Redwine: Roles and permissions and Groups fixes */ elseif (($privacy != 'private' || isMemberActive($requestID) == 'active')) {
...
/***************************
if we are testing the smtp email send, WITH A FAKE EMAIL ADDRESS, the SESSION variable will allow us to print the email message when the register_complete.php is loaded, so that the account that is created can be validated and activated.
 ***************************/
...
// for the private group joining email
if (allow_smtp_testing == 1 && smtp_fake_email == 1) {
